package com.mycompany.prepare;


/**
 * Thread interruption (Interrupt)
 * Gain and release Lock for sleep and wait (LockReturnOnWaitSleep)
 * MultipleSyncEntry (return lock, StackOverflow, DoubleSyncEntry)
 * Volatile usages (AppVolatile)
 * Singleton realization (SingletonVolatile)
 * ThreadLocal (ThreadLocalEx)
 *
 * Lock interface (External Lock) - advantageous
 * Examples: ReentrantLock1,2 (liveness)
 *
 *
 *
 * Dispute on Thread Safe objects: Immutable objects Mutability, defend from concurrent object access
 */
public class App {
}
