...
import com.epam.metrics.connectors.rest.jira.models.sprints.SprintReport;
import com.epam.metrics.connectors.rest.jira.models.sprints.Sprints;
...
public class JiraClient {
    ...
    public JiraClient(HttpClient httpClient, String uri, ICredentials creds) throws JiraException {
        ...
        restclient = new JiraRestClient(httpClient, creds, URI.create(uri));
    ...
    /// <summary>
    /// Returns all SprintReports containing Sprints inside with Start/End dates
    /// </summary>
    /// <param name="rapidViewId">The rapid view id.</param>
    /// <returns>Sprints for specified rapid view.</returns>
    public List<SprintReport> getSprintReports(int rapidViewId) throws URISyntaxException, IOException, RestException, InterruptedException {

        Sprints sprints = getSprintsWithoutDates(rapidViewId);

        List<SprintReport> sprintReports = new ArrayList<>();
        ExecutorService es = Executors.newFixedThreadPool(50); // /adjustable

        // make concurrent Rest request (each request can take 1-2 sec, so for 100 Sprints it can take 2-3 min in one only thread)
        List<Callable<Object>> jobs = new ArrayList<>(sprints.getSprints().size());

        for (Sprint sprint : sprints.getSprints()) {

            jobs.add(Executors.callable((PrivilegedAction<?>) () -> {
                        try {
                            return getJiraSprintReport(rapidViewId, sprint.getId());

                        } catch (IOException | URISyntaxException | RestException e) {
                            log.error("Rest request to Jira Error", e);
                            e.printStackTrace();
        ...
        List<Future<Object>> futures = es.invokeAll(jobs);
        for (Future<?> future : futures) {
            try {
                sprintReports.add((SprintReport) future.get());
    ...
    // It will return the whole Sprint report (see example: 'sprintReport.json')
    public SprintReport getJiraSprintReport(int rapidViewId, int sprintId) throws IOException, URISyntaxException, InterruptedException, RestException {

        ResponseEntity<SprintReport> response = restclient.get("/rest/greenhopper/1.0/rapid/charts/sprintreport?rapidViewId=" + rapidViewId + "&sprintId=" + sprintId, SprintReport.class);

        return JiraUtil.validateResponse(response, HttpStatus.OK);
    }

    /// <summary>
    /// Returns sprints for the specified rapid view without Start/End dates
    /// Quick single request
    /// </summary>
    /// <param name="rapidViewId">The rapid view id.</param>
    /// <returns>Sprints for specified rapid view.</returns>
    public Sprints getSprintsWithoutDates(int rapidViewId) throws URISyntaxException, IOException, RestException, InterruptedException {

        Map<String, String> params = new HashMap<>();
        params.put("includeNonClosedPeriods", "true");

        ResponseEntity<Sprints> response = restclient.get("/rest/greenhopper/1.0/sprintquery/" + rapidViewId, params, Sprints.class);

        return JiraUtil.validateResponse(response, HttpStatus.OK);
    }

    /// <summary>
    /// Returns sprints with dates for the specified rapid view.
    /// Heavily loaded multiple Rest requests. Can take 3-5 sec to execute for 100-200 sprints in 50 threads.
    /// </summary>
    /// <param name="rapidViewId">The rapid view id.</param>
    /// <returns>Sprints for specified rapid view.</returns>
    public Sprints getSprintsWithDates(int rapidViewId) throws URISyntaxException, IOException, RestException, InterruptedException {

        final Sprints sprintsWithDates = new Sprints();

        List<SprintReport> sprintReports = getSprintReports(rapidViewId);

        sprintReports.forEach(r -> sprintsWithDates.getSprints().add(r.getSprint()));

        return sprintsWithDates;
    }
...
//    }
    ...
    private JSON getNextPortion(Issue issue, Integer startAt)
        ...
        URI uri = restclient.buildURI(Issue.getBaseUri() + "issue/" + issue.getId(), params);
     ...
     * NB: Greeenhopper
     */
    public SprintReport getSprintReport(int rapidViewId, int sprintId, SearchOptions options) throws IOException, URISyntaxException {
        ResponseEntity<SprintReport> response = restclient.get("/rest/greenhopper/1.0/rapid/charts/sprintreport?rapidViewId={rapidViewId}&sprintId={sprintId}", SprintReport.class);
        SprintReport result = JiraUtil.validateResponse(response, HttpStatus.OK);
        //TODO: add statistics!
        return result;
    }
}
